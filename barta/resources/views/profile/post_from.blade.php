      ...
      <div class="flex items-start /space-x-3/">
        <!-- User Avatar -->
          <div class="flex-shrink-0">
            <img class="h-10 w-10 rounded-full object-cover" src="{{ asset('storage/'.Auth::user()->photo) }}" alt="">
          </div>
        <!-- /User Avatar -->

        <!-- Content -->
        <div class="text-gray-700 font-normal w-full">
          <textarea class="block w-full p-2 pt-2 text-gray-900 rounded-lg border-none outline-none focus:ring-0 focus:ring-offset-0" name="barta" rows="2" placeholder="What's going on, {{ auth()->user()->username }}?"></textarea>
        </div>
      </div>
    </div>
      ...
      <div class="flex items-center justify-between">
        <div class="flex gap-4 text-gray-600">
          ...
          <label for="image" class="-m-2 flex gap-2 text-xs items-center rounded-full p-2 text-gray-600 hover:text-gray-800 cursor-pointer">
            <input type="file" name="image" id="image" class="hidden" @change="src = URL.createObjectURL(event.target.files[0])">
            <img style="width: 80px; height:80px; object-fit:cover" id="image" :src="src" alt=""/>
